        ...
        <section className="py-24 bg-slate-50 dark:bg-slate-950 relative overflow-hidden transition-colors duration-500">
            ...
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
                <motion.div
                    ...
                    className="absolute top-0 right-0 w-[600px] h-[600px] bg-sky-100/50 dark:bg-sky-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"
                />
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div>
                        ...
                        >
                            <span className="inline-block py-1.5 px-4 rounded-full bg-white dark:bg-slate-900 border border-sky-100 dark:border-slate-800 text-sky-600 dark:text-sky-400 text-[10px] font-bold tracking-widest uppercase shadow-sm mb-6">
                                ✨ Why We Are The Best
                            </span>
                    ...
                    <div className="relative h-[600px] w-full flex items-center justify-center">
                        ...
                        <motion.div
                            ...
                            className="relative z-10 w-full h-[500px] rounded-3xl overflow-hidden shadow-2xl border-4 border-white dark:border-slate-800"
                        >
                            <img
                        ...
                        <motion.div
                            ...
                            className="absolute -top-4 -right-4 md:right-8 z-20 bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 flex flex-col items-center gap-2"
                        >
                            ...
                            <div className="text-center">
                                ...
                                <span className="text-xs font-bold text-slate-500 dark:text-slate-500 uppercase">Safety Record</span>
                            </div>
                        </motion.div>
                        ...
                        <div className="absolute bottom-8 -left-4 md:left-8 z-30 max-w-xs w-full">
                            <AnimatePresence mode='wait'>
                                <motion.div
                                    ...
                                    className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl p-5 rounded-2xl shadow-2xl border border-white/50 dark:border-slate-800/50"
                                >
                                    {/* Stars */}
                                    ...
                                    {/* Text */}
                                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300 italic mb-4 min-h-[60px]">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <p className="text-sm font-bold text-slate-900 dark:text-white">{reviews[currentReview].name}</p>
                ...
                >
                    {stats.map((stat) => (
                        <div key={stat.id} className="bg-white dark:bg-slate-900/50 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-lg dark:hover:shadow-sky-500/5 hover:-translate-y-1 transition-all duration-300 group text-center">
                            <div className={`w-14 h-14 mx-auto rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${stat.bg} dark:bg-opacity-10 ${stat.color}`}>
                                {stat.icon}
                            </div>
                            <h3 className="text-3xl font-black text-slate-900 dark:text-white mb-1">{stat.value}</h3>
                            <p className="text-sm font-semibold text-slate-500 dark:text-slate-500 uppercase tracking-wide">{stat.label}</p>
                        </div>
                    ))}
                </motion.div>
...
const FeatureCard = ({ title, desc, icon, delay }) => (
    <motion.div
        ...
        viewport={{ once: true }}
        transition={{ delay }}
        className="group flex items-start gap-4 p-4 rounded-2xl hover:bg-white dark:hover:bg-slate-900 hover:shadow-xl dark:hover:shadow-sky-500/5 hover:shadow-sky-100/50 border border-transparent hover:border-sky-50 dark:hover:border-slate-800 transition-all duration-300 cursor-default"
    >
        <div className="mt-1 w-12 h-12 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-center group-hover:scale-110 group-hover:bg-sky-50 dark:group-hover:bg-slate-800 transition-all duration-300 shrink-0">
            {icon}
        </div>
        <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">{title}</h3>
            <p className="text-sm text-slate-500 dark:text-slate-500 leading-relaxed group-hover:text-slate-600 dark:group-hover:text-slate-400">{desc}</p>
        </div>
    </motion.div>
);
