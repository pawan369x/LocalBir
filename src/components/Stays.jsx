                    ...
                    <div className="flex items-center justify-between mt-auto">
                        <ul className="flex gap-2">
                            {stay.amenities.map((item, i) => (
                                <li key={i} className="bg-slate-50 border border-slate-100 text-slate-500 px-2 py-1 rounded-lg text-[10px] flex items-center gap-1 font-semibold">
...
const BookingModal = ({ stay, onClose }) => {
    ...
    const handleWhatsApp = () => {
        const message = `Hi Bir Billing Guide! I want to book a package.%0A%0A*Stay:* ${stay.name} (${stay.displayPrice})%0A*Name:* ${name}%0A*Date:* ${date}%0A*Guests:* ${guests}%0A*Paragliding:* ${addParagliding ? `Yes (${guests} Pax)` : 'No'}%0A%0A*Total Est:* ₹${grandTotal}`;
        window.open(`https://wa.me/916230044384?text=${message}`, '_blank');
    };
