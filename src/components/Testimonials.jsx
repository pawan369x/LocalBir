    ...
    return (
        <section className="py-24 bg-white dark:bg-slate-950 relative overflow-hidden transition-colors duration-500">
            ...
            <div className="text-center mb-16 relative z-10 px-4">
                <span className="text-sky-500 dark:text-sky-400 font-bold tracking-widest uppercase text-sm">Wall of Love</span>
            ...
            <div className="absolute left-0 top-0 bottom-0 w-20 md:w-40 bg-gradient-to-r from-white dark:from-slate-950 to-transparent z-20 pointer-events-none" />
            <div className="absolute right-0 top-0 bottom-0 w-20 md:w-40 bg-gradient-to-l from-white dark:from-slate-950 to-transparent z-20 pointer-events-none" />
...
const ReviewCard = ({ review }) => {
    return (
        <div className="w-80 md:w-96 bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 flex-shrink-0 hover:shadow-xl dark:hover:shadow-sky-500/5 hover:scale-[1.02] transition-all duration-300 cursor-pointer select-none group">
            <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                    <img src={review.img} alt={review.name} className="w-10 h-10 rounded-full border-2 border-white dark:border-slate-800 shadow-sm object-cover" />
                    <div>
                        <h4 className="font-bold text-slate-900 dark:text-white text-sm group-hover:text-sky-500 transition-colors">{review.name}</h4>
                        <p className="text-xs text-slate-400 dark:text-slate-500">{review.loc}</p>
                    </div>
                </div>
                <Quote size={20} className="text-sky-200 dark:text-sky-900 fill-sky-100 dark:fill-sky-500/10" />
            </div>

            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed mb-4 italic">"{review.text}"</p>
