...
const LiveStatus = ({ scrolled }) => (
    <div className={`bg-sky-50 dark:bg-slate-900/50 text-sky-900 dark:text-sky-300 flex items-center justify-center text-xs md:text-sm border-b border-sky-100 dark:border-slate-800 relative z-[60] overflow-hidden transition-all duration-300 ${scrolled ? 'md:h-0 md:py-0 md:border-b-0 md:opacity-0 pointer-events-none' : 'h-9 border-b'}`}>
...
const Navbar = ({ onBookClick }) => {
    ...
    const textColor = isTransparent ? 'text-white' : 'text-slate-800 dark:text-slate-200';
    ...
    return (
        <header className="fixed w-full top-0 z-50 flex flex-col font-sans">
            ...
            <LiveStatus scrolled={scrolled} />

            <motion.nav
                className={`w-full transition-all duration-300 ${!isTransparent ? 'bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800 py-3 shadow-sm' : 'bg-transparent py-5'
                    }`}
            >
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="hidden lg:flex items-center gap-4">
                            ...
                            >
                                Book My Flight <ChevronRight size={16} />
                            </motion.button>
                        </div>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        animate="animate"
                        exit="exit"
                        className="fixed inset-0 bg-white dark:bg-slate-950 z-40 flex flex-col lg:hidden pt-24 px-6 h-screen"
                    >
                        <motion.div
                        ...
                        >
                            ...
                            <motion.div variants={linkVars} className="mt-auto mb-8">
                                ...
                                >
                                    Claim My Wings <ArrowRight size={20} />
                                </button>
