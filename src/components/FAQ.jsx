    ...
    return (
        <section className="py-24 bg-white dark:bg-slate-950 relative overflow-hidden transition-colors duration-500">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <div className="absolute top-40 left-10 w-96 h-96 bg-sky-100 dark:bg-sky-500/5 rounded-full mix-blend-multiply filter blur-3xl opacity-60 animate-blob" />
                <div className="absolute bottom-40 right-10 w-96 h-96 bg-blue-100 dark:bg-blue-500/5 rounded-full mix-blend-multiply filter blur-3xl opacity-60 animate-blob animation-delay-2000" />
            </div>
                    ...
                    <div className="lg:col-span-4 lg:sticky lg:top-24">
                        ...
                        >
                            <div className="flex items-center gap-2 mb-6">
                                <span className="p-2 bg-sky-100 dark:bg-sky-500/10 rounded-lg text-sky-600 dark:text-sky-400">
                                    <HelpCircle size={24} />
                                </span>
                                <span className="text-sky-600 dark:text-sky-400 font-bold tracking-wider uppercase text-sm">
                                    Support Center
                                </span>
                            </div>
                            ...
                            {faqs.map((faq, index) => (
                                <motion.div
                                    ...
                                    className={`group border rounded-2xl transition-all duration-300 overflow-hidden ${activeId === faq.id
                                        ? 'border-sky-500 bg-sky-50/30 dark:bg-sky-500/10 shadow-lg shadow-sky-100 dark:shadow-none' 
                                        : 'border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/50 hover:border-sky-200 dark:hover:border-sky-500/40 hover:shadow-md'
                                        }`}
                                >
                                    <button
                                    ...
                                    >
                                        <span className={`text-lg font-bold pr-8 transition-colors ${activeId === faq.id ? 'text-sky-600 dark:text-sky-400' : 'text-slate-800 dark:text-slate-200 group-hover:text-sky-600 dark:group-hover:text-sky-400'
                                            }`}>
                                            {faq.question}
                                        </span>
                                        <span className={`flex-shrink-0 p-2 rounded-full transition-all duration-300 ${activeId === faq.id
                                            ? 'bg-sky-500 text-white rotate-180 shadow-lg shadow-sky-500/20'
                                            : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 group-hover:bg-sky-100 dark:group-hover:bg-sky-500/20 group-hover:text-sky-500'
                                            }`}>
                                        ...
                                        {activeId === faq.id && (
                                            ...
                                            >
                                                <div className="px-6 pb-6 pt-0">
                                                    <div className="h-px w-full bg-sky-100 dark:bg-sky-500/10 mb-4" /> {/* Sky Divider */}
                                                    <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-base italic">
                                                        {faq.answer}
                                                    </p>
                                                </div>
                                            </motion.div>
